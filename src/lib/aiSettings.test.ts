...
describe("aiSettings Ollama defaults", () => {
  it("uses the Ollama native API root instead of the OpenAI-compatible v1 path", () => {
    expect(BUILTIN_PROVIDERS.ollama?.defaultBaseUrl).toBe(
      "http://localhost:11434/",
    );

    const profile = DEFAULT_AI_SETTINGS.provider_profiles.find(
      (item) => item.id === "ollama",
    );
...
describe("External MCP defaults", () => {
  it("starts disabled with a temporary confirm-scoped server", () => {
